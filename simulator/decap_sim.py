...
def decap_placement(n,m, raw_pdn, pi, probing_port,freq_pts):
    num_decap=np.size(pi)
    probe=probing_port
    z1=raw_pdn


    with open('simulator/EM_model/01nF_decap.npy', 'rb') as f: 
        decap = np.load(f)
...
def decap_model(z_initial, z_final,N_freq):

    impedance_gap=np.zeros(N_freq)

    with open ('simulator/EM_model/freq_201.npy', 'rb') as f:
        freq=np.load(f)
...
def decap_sim(probe, solution,keepout=None,N=10,N_freq=201):
    
    with open ('simulator/EM_model/10x10_pkg_chip.npy', 'rb') as f:
        raw_pdn=np.load(f)
    solution = np.array(solution)
    
    
    assert len(solution)==len(np.unique(solution)), "An Element of Decap Sequence must be Unique"
